...
namespace TranslatoryParser.Common
{
    public class Parser
    {
        ...
        static readonly Parser<Expression> Constant = Parse.Decimal
        ...
        static readonly Parser<Expression> Operand = (from sign in Parse.Char('-') from factor in Factor select Expression.Negate(factor))
            .XOr(Factor)
            .Token();
        ...
        static readonly Parser<Expression<Func<double>>> Lambda = Expr.End().Select(body => Expression.Lambda<Func<double>>(body));
    }
}
